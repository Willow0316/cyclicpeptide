...
def cal_aa_composition():
    ...
    msg = pd.DataFrame([['Tiny', '(A+C+G+S+T) '], ['Small', '(A+B+C+D+G+N+P+S+T+V)'], ['Aliphatic', '(A+I+L+V)'],
                        ...
                        ['Basic', '(H+K+R)'], ['Acidic', '(B+D+E+Z)']], columns=['Property', 'Residues'],
                       index=range(9))
...
def predict_properties_from_sequence(sequence):
    ...
    # 16. protFP
    try:
        protFP = pd.DataFrame(r("protFP(sequence)")[0], index=['ProtFP' + str(i) for i in range(1, 9)],
                              columns=['Indices'])
    except:
        protFP = pd.DataFrame()
    # 17. stScales
    try:
        stScales = pd.DataFrame(r("stScales(sequence)")[0], index=['ST' + str(i) for i in range(1, 9)],
                                columns=['Indices'])
    except:
        stScales = pd.DataFrame()
    # 18. tScales
    try:
        tScales = pd.DataFrame(r("tScales(sequence)")[0], index=['T' + str(i) for i in range(1, 6)],
                               columns=['Indices'])
    except:
    ...
    # 20. zScales
    try:
        zScales = pd.DataFrame(r("zScales(sequence)")[0], index=['Z' + str(i) for i in range(1, 6)],
                               columns=['Indices'])
    except:
